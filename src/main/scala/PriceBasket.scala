...
trait PriceBasket {
  ...
  def validateBasket(input: List[String]): Either[Error, List[Item]] = {
    input
      .map(FactoryHelper.getItem).partition(_.isLeft) match {
  ...
  def processItems(input: Map[Item, Int])(implicit offers: List[Offers]): Output = {
    ...
    val subTotal: BigDecimal = allItems.map(_.subTotal).sum
    val discountText: String = itemsWithOffers.flatten.map(x => s"${x.offerTitle} : ${x.subTotal - x.total}").mkString("\n")
    val totalPrice: BigDecimal = allItems.map(_.total).sum
